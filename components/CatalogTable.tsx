    ...
    <div>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full">
          <thead><tr>
            <th className="th w-8"><input type="checkbox" className="accent-[#C89B3C]" checked={sel.size > 0 && sel.size === rows.length} onChange={toggleAll} /></th>
            ...
            {rows.map((it) => {
              ...
              return (
                <tr key={it.id} className={`hover:bg-navy3/50 ${sel.has(it.id) ? "bg-navy3/40" : ""}`}>
                  <td className="td"><input type="checkbox" className="accent-[#C89B3C]" checked={sel.has(it.id)} onChange={() => toggle(it.id)} /></td>
