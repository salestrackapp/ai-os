  ...
  return (
    <>
      <button onClick={() => setOpen(true)}
        className="ds-focus inline-flex h-10 items-center gap-1.5 rounded-ds-input border border-hairline px-3 font-montserrat text-[13px] font-medium text-[color:var(--fg-2)] transition-colors hover:bg-[var(--bg-2)]">
        ...
        <div className="space-y-5">
            ...
            <ol className="space-y-3">
              {help.passos.map((p, i) => (
                <li key={i} className="flex gap-3">
                  <span className="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--tile)] font-jbmono text-[11px] font-medium text-[color:var(--brand-deep)]">{i + 1}</span>
          ...
          {help.dica && (
            <div className="rounded-ds-input border border-hairline bg-[var(--bg-2)] p-3.5">
              ...
              <p className="font-montserrat text-[13px] leading-snug text-[color:var(--fg-2)]">{help.dica}</p>
            </div>
          )}
          <a href={routeKey.startsWith("/portal") ? "/portal/ajuda" : "/admin/ajuda"}
            className="inline-flex items-center gap-1.5 font-montserrat text-[13px] font-medium text-[color:var(--brand)] hover:underline">
