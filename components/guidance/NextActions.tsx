      ...
      <div className={`grid gap-3 ${items.length > 1 ? "lg:grid-cols-3" : ""}`}>
        {items.map((s, i) => (
          <Link key={i} href={s.href}
            className="ds-focus group flex items-center gap-3 rounded-ds-card border border-hairline bg-[var(--bg-1)] p-4 shadow-ds-xs transition-all duration-200 hover:-translate-y-0.5 hover:border-[color:rgba(0, 122, 148,0.28)] hover:shadow-ds-md">
            ...
            <span className="min-w-0 flex-1">
              <span className="block font-montserrat text-[14px] font-medium leading-snug text-[color:var(--fg-1)]">{s.finding}</span>
              <span className="mt-1 inline-flex items-center gap-1 font-montserrat text-[13px] font-semibold text-[color:var(--brand)]">{s.label} <span className="transition-transform group-hover:translate-x-0.5">→</span></span>
            </span>
          </Link>
        ))}
      </div>
