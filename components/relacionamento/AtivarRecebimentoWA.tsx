  ...
  return (
    <div className="mb-4 rounded-ds-card border border-hairline bg-[var(--bg-2)] p-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="font-montserrat text-[14px] text-[color:var(--fg-1)]"><b>Recebimento automático (real-time).</b> Registra o webhook da Z-API para as mensagens caírem aqui sozinhas.</p>
        <button disabled={pending} onClick={() => start(async () => { setRes(await ativarRecebimentoWhatsAppAction()); })}
          className={botaoClasses({ className: "shrink-0 disabled:opacity-50" })}>
          {pending ? "Registrando…" : "Ativar recebimento"}
        </button>
      </div>

      {res?.ok && <p className="mt-2 break-all font-montserrat text-[13px] text-[color:var(--brand-deep)]">✓ Webhook registrado: {res.url}</p>}
      ...
      {res && !res.ok && (
        <div className="mt-2 rounded-ds-input bg-[#FDECEC] p-3">
          ...
          {precisaToken && (
            <div className="mt-2 space-y-2 font-montserrat text-[13px] text-[color:var(--fg-1)]">
              ...
              {res.url && (
                <div className="flex items-center gap-2">
                  <code className="min-w-0 flex-1 truncate rounded bg-[var(--bg-1)] px-2 py-1 font-jbmono text-[13px] text-[color:var(--fg-2)]">{res.url}</code>
                  <button onClick={() => copiar(res.url!)} className="ds-focus shrink-0 rounded-ds-input border border-hairline-strong px-2.5 py-1 text-[13px] text-[color:var(--fg-2)] hover:bg-[var(--bg-2)]">{copied ? "copiado!" : "copiar"}</button>
                </div>
              )}
            </div>
          )}
