...
const lbl = "mb-1 block font-montserrat text-[14px] font-medium text-[color:var(--fg-2)]";
...
export function NovaJornadaForm({ ofertas }: { ofertas: string[] }) {
    ...
    return (
      <div className="mx-auto max-w-xl">
        <div className="rounded-ds-card border border-[color:var(--brand-light)] bg-[var(--tile)] p-6 text-center">
          ...
          <p className="mt-1 font-montserrat text-[14px] text-[color:var(--fg-2)]">Cliente, oportunidade e diagnóstico prontos. Envie o link abaixo para começar.</p>
        </div>
        <div className="mt-4 rounded-ds-card border border-hairline bg-[var(--bg-1)] p-4">
          <p className={lbl}>Link do diagnóstico</p>
          <div className="flex items-center gap-2">
            <code className="min-w-0 flex-1 truncate rounded-ds-input border border-hairline bg-[var(--bg-2)] px-3 py-2.5 font-jbmono text-[13px] text-[color:var(--fg-2)]">{link}</code>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button disabled={sending} onClick={() => startSend(async () => { const r = await enviarDiagWhatsAppAction(done.orgId, done.token); setEnvio(r.ok ? { tone: "ok", text: "Enviado por WhatsApp ✓" } : { tone: "err", text: r.erro ?? "Falha no WhatsApp." }); })}
              className={botaoClasses({ className: "disabled:opacity-50" })}>Enviar por WhatsApp</button>
          ...
          {envio && <p className={`mt-2 font-montserrat text-[13px] ${envio.tone === "ok" ? "text-[color:var(--brand-deep)]" : "text-[#B42318]"}`}>{envio.text}</p>}
        </div>
        <div className="mt-4 flex gap-2">
          <Link href={`/admin/clientes/${done.orgId}`} className={botaoClasses()}>Abrir a jornada</Link>
  ...
  return (
    <form onSubmit={submit} className="mx-auto max-w-xl space-y-5">
      ...
      <p className="-mt-2 font-montserrat text-[13px] text-[color:var(--fg-4)]">Informe pelo menos um: WhatsApp ou e-mail (para enviar o diagnóstico).</p>
      <div>
      ...
      {erro && <p className="rounded-ds-input bg-[#FDECEC] px-3 py-2 font-montserrat text-[14px] text-[#B42318]">{erro}</p>}
      <div className="flex items-center gap-3 pt-1">
        <button disabled={pending} className={botaoClasses({ size: "lg", className: "disabled:opacity-50" })}>{pending ? "Criando…" : "Criar jornada"}</button>
        <span className="font-montserrat text-[13px] text-[color:var(--fg-4)]">Cria empresa + contato + oportunidade + diagnóstico. Contrato e proposta entram depois, dentro da jornada.</span>
      </div>
    </form>
  );
}
