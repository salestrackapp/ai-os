/**
 * DS v5 · Card (Salestrack AI v2)
 * Card — superfície branca, hairline, sombra suave, tile de ícone lavanda no topo.
 * CardFeatured — fundo gradiente violeta→índigo, texto branco (item atual/recomendado/flagship).
...
export function Card({ icon, eyebrow, title, children, footer, bloom, className }: {
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-ds-card border border-hairline bg-[var(--bg-1)] p-6 shadow-ds-sm", className)}>
...
export function CardFeatured({ icon, eyebrow, title, children, footer, className }: {
  ...
  return (
    <div className={cn("relative overflow-hidden rounded-ds-card p-6 text-white shadow-ds-brand", className)} style={{ background: "var(--grad-brand)" }}>
      ...
      {eyebrow && <p className="mb-2 inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-white/85"><span className="h-px w-4 bg-white/70" />{eyebrow}</p>}
