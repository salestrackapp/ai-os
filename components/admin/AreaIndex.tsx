...
function SlotButton({ icon, label }: { icon: string; label: string }) {
  return (
    <span title="Disponível em breve" aria-disabled
      className="inline-flex h-10 cursor-default items-center gap-1.5 rounded-ds-input border border-hairline px-3 font-montserrat text-[14px] font-medium text-[color:var(--fg-4)]">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {a.sections.map((s) => (
            <Link key={s.href} href={s.href} className="group block">
              <div className="h-full rounded-ds-card border border-hairline bg-[var(--bg-1)] p-5 shadow-ds-sm transition-all duration-200 group-hover:-translate-y-0.5 group-hover:border-[color:rgba(0, 122, 148,0.28)] group-hover:shadow-ds-md">
                ...
                <span className="mt-3 inline-flex items-center gap-1 font-montserrat text-[13px] font-medium text-[color:var(--brand)]">Abrir <span className="transition-transform group-hover:translate-x-0.5">→</span></span>
              </div>
            </Link>
          ))}
        </div>
