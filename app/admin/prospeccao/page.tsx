...
export default async function ProspeccaoPage({ searchParams }: { searchParams: Promise<{ icp?: string; status?: string; page?: string }> }) {
  ...
  const apolloOn = await apolloConfigured();
        ...
        <div className="grid lg:grid-cols-2 gap-5 mb-6">
          <div className="card p-6">
            ...
            <form action={importPasted} className="space-y-2">
              <p className="text-[13px] text-muted2">Cabeçalho flexível: <span className="font-mono">name, title, email, company, domain, icp, linkedin, seniority, industry, size, signals</span> (sinais separados por “;”).</p>
          ...
          <div className="card p-6">
            ...
            <form action={importFromApollo} className="mt-3 flex flex-wrap items-center gap-2 pt-3 border-t border-line">
              ...
              <button className="btn-ghost text-xs" disabled={!apolloOn} title={apolloOn ? "" : "Configure a chave do Apollo no Console"}>Buscar no Apollo</button>
              {!apolloOn && <span className="text-[13px] text-muted2">Apollo em modo manual (sem chave)</span>}
            </form>
          </div>
        </div>
              ...
              {list.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-navy3/50 border-b border-line last:border-0">
                    <td className="td text-cream">{p.name}<span className="block text-[13px] text-muted2">{p.title ?? ""}</span></td>
                    <td className="td text-muted">{acc?.name ?? "—"}{acc?.signals?.length ? <span className="block text-[11px] text-gold">⚡ {acc.signals.length} sinal(is)</span> : null}</td>
