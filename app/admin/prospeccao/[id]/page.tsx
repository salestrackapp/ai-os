      ...
      <div className="grid lg:grid-cols-3 gap-5">
              ...
              <ul className="space-y-2">
                {timeline.map((e) => (
                  <li key={e.id} className="flex gap-3 text-sm border-b border-line last:border-0 pb-2">
                    <span className="text-[11px] text-muted2 w-24 shrink-0">{e.occurred_at ? new Date(e.occurred_at).toLocaleDateString("pt-BR") : ""}</span>
        ...
        <div className="space-y-5">
          <div className="card p-6">
            ...
            <p className="text-[11px] text-muted2 mt-2">Toques entram como rascunho na fila de aprovação — nada é enviado sem sua revisão.</p>
          </div>
          ...
          <div className="card p-6">
            ...
            {(enrollments ?? []).length > 0 && (
              <div className="mt-3 space-y-1">
                {(enrollments ?? []).map((e) => <p key={e.id} className="text-[11px] text-muted2">Passo {e.current_step} · {e.status}{e.next_action_at ? ` · próximo ${new Date(e.next_action_at).toLocaleDateString("pt-BR")}` : ""}</p>)}
              </div>
            )}
          </div>
