...
const RISK_BADGE: Record<string, string> = { baixo: "badge-teal", medio: "badge-gold", alto: "badge inline-flex text-[10px] uppercase tracking-[.14em] px-2.5 py-1 rounded-full border text-red-400 border-red-500/40 bg-red-500/10" };
        ...
        <div className="card p-6"><p className="label mb-3">Preços de modelo (USD/1M tokens)</p>
          {(prices ?? []).map((p) => (
            <form key={p.id} action={saveModelPrice.bind(null, p.id)} className="flex items-center gap-2 py-1">
              ...
              <button className="btn-ghost text-[11px]">ok</button>
            </form>
          ))}
        </div>
      </div>
          ...
          {(alerts ?? []).map((a) => (
            <div key={a.id} className="card p-4 flex items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-2">
                <span className={a.severity === "critico" ? "badge inline-flex text-[10px] uppercase tracking-[.14em] px-2.5 py-1 rounded-full border text-red-400 border-red-500/40 bg-red-500/10" : "badge-gold"}>{a.severity}</span>
                <span className="text-sm text-cream">{a.message}</span>
                <span className="text-[11px] text-muted2">{a.kind} · {a.status}</span>
              </div>
