...
import { PageHeader, botaoClasses, ContentArea } from "@/components/ds";
...
export default async function CatalogoPage({ searchParams }: { searchParams: Promise<{ kind?: string; brand?: string }> }) {
  ...
  const chip = (active: boolean) =>
    `rounded-ds-pill border px-3 py-1 font-montserrat text-[13px] transition-colors ${active ? "border-[color:var(--brand)] bg-[var(--tile)] text-[color:var(--brand-deep)]" : "border-hairline text-[color:var(--fg-3)] hover:border-[color:var(--brand-light)]"}`;
        ...
        <PageHeader eyebrow="Comercial" title="Catálogo de ofertas"
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              <Link href="/admin/catalogo/novo" className={botaoClasses()}><Icon name="gem" size={15} /> Nova oferta</Link>
            </div>
          } />
