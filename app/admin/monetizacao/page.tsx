...
export default async function MonetizacaoPage() {
  ...
  return (
    <div>
      <div className="mb-6 flex items-end justify-between gap-4 flex-wrap"><div><p className="text-[11px] uppercase tracking-[.24em] text-muted2 mb-1">Monetizar</p><h1 className="font-serif text-4xl font-semibold">Central de Monetização</h1>
          ...
          {planList.map((p) => (
            <form key={p.id} action={savePlan.bind(null, p.id)} className="card p-5 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-1 pt-1">
                {BOOL_FEATS.map((f) => <label key={f} className="flex items-center gap-1.5 text-[11px] text-muted"><input type="checkbox" name={`f_${f}`} defaultChecked={!!p.features?.[f]} /> {f}</label>)}
              </div>
              <div className="flex gap-2">
              ...
              <div key={o.id} className="card p-6">
                ...
                <div className="grid lg:grid-cols-3 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Link href={`/admin/programas`} className="text-[11px] text-muted2 hover:text-gold block">O cliente edita a política no portal → Governança</Link>
                  </div>
                </div>
              </div>
            );
