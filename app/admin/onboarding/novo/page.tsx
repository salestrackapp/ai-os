        ...
        <form action={provisionAction} className="space-y-6">
          ...
          <div className="card p-6 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div><label className="label">Cor de acento</label><input name="color_accent" className="input w-full" placeholder="#007A94" /></div>
          ...
          <div className="card p-6 space-y-3">
            ...
            <p className="text-[13px] text-gold mt-1"><Icon name="sparkles" size={12} className="inline -mt-0.5" /> Recomendado: <b>{(templates ?? []).find((t) => t.key === rec.templateKey)?.name ?? rec.templateKey}</b> — {rec.justificativa}{rec.fallback ? " (regra de fallback)" : ""}</p>
            <p className="text-[13px] text-muted2">O template monta projeto, entregáveis, timeline e biblioteca inicial. Sem template → programa mínimo válido.</p>
          </div>
          ...
          <div className="card p-6 space-y-3">
            ...
            <p className="text-[13px] text-muted2">{(await googleConfigured()) ? "O convite será enviado por e-mail (conta Salestrack)." : "Sem Gmail: o link do convite aparece na Central de Onboarding para você copiar."} A senha é definida pelo próprio usuário.</p>
          </div>
