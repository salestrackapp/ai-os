...
const ST_BADGE: Record<string, string> = { pronto: "badge-teal", provisionando: "badge-gold", pendente: "badge-muted", falhou: "badge inline-flex text-[11px] uppercase tracking-[.14em] px-2.5 py-1 rounded-full border text-red-400 border-red-500/40 bg-red-500/10" };
              ...
              <div key={p.id} className="card p-5">
                <div className="flex items-center justify-between gap-3 mb-2 flex-wrap">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[13px] text-muted2">{p.source}{p.template_key ? ` · ${p.template_key}` : ""} · {new Date(p.created_at).toLocaleString("pt-BR")}</span>
                  </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(p.steps ?? []).map((s, i) => <span key={i} className={s.status === "pronto" ? "badge-teal" : s.status === "falhou" ? "badge inline-flex text-[11px] uppercase tracking-[.14em] px-2.5 py-1 rounded-full border text-red-400 border-red-500/40 bg-red-500/10" : "badge-muted"} title={s.erro ?? ""}>{s.status === "pronto" ? "✓" : s.status === "falhou" ? "✗" : "·"} {s.passo}</span>)}
                </div>
              </div>
            ))}
              ...
              <tbody>
                {((invites as Inv[]) ?? []).map((i) => (
                  <tr key={i.id} className="hover:bg-navy3/50 border-b border-line last:border-0">
                    ...
                    <td className="td"><span className="text-[13px] text-muted2 break-all">{base}/convite/{i.token.slice(0, 8)}…</span></td>
            ...
            {(orgs ?? []).map((o) => {
              ...
              return (
                <div key={o.id} className="card p-5">
                  ...
                  {c?.completed_at && <p className="text-[13px] text-teal-300 mt-1">✓ Ativação concluída</p>}
                </div>
              );
            })}
          </div>
