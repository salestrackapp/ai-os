...
export default async function ContaDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: deals }, { data: contacts }] = await Promise.all([
    supabase.from("deals").select("*").eq("org_id", id).is("deleted_at", null).order("created_at", { ascending: false }),
    supabase.from("contacts").select("*").eq("org_id", id).is("deleted_at", null).order("name"),
  ]);
            ...
            <div className="card p-6">
              ...
              <div className="space-y-2 mb-4">
                {(contacts as Contact[] | null)?.map((c) => (
                  <div key={c.id} className="text-sm flex items-center justify-between gap-2">
                    <div><p className="text-cream">{c.name}{c.role && <span className="text-muted2 text-xs"> · {c.role}</span>}</p>{c.email && <p className="text-[13px] text-muted2">{c.email}</p>}</div>
              ...
              <details className="border-t border-line pt-3">
                ...
                <form action={addContactToOrg.bind(null, id)} className="mt-3 space-y-2">
                  ...
                  <label className="flex items-center gap-2 text-xs text-muted"><input type="checkbox" name="opt_in_whatsapp" className="accent-[#007A94]" /> Opt-in WhatsApp</label>
