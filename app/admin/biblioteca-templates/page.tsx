          ...
          {((tpls as Tpl[]) ?? []).map((t) => {
            ...
            return (
              <div key={t.key} className="card p-5">
                ...
                <p className="text-[11px] text-muted2">{dels} entregáveis · {tl} fases · {vs.length} versão(ões)</p>
                ...
                {vs.length > 0 && <div className="mt-2 pt-2 border-t border-line space-y-0.5">{vs.slice(0, 4).map((v) => <p key={v.version} className="text-[11px] text-muted2">v{v.version}{v.is_published ? " · publicada" : ""}{v.changelog ? ` — ${v.changelog}` : ""}</p>)}</div>}
              </div>
            );
          })}
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-1">
            {catOrder.flatMap((cat) => blockList.filter((b) => b.category === cat)).map((b) => (
              <label key={b.key} className="flex items-center gap-2 text-xs text-muted">
                <input type="checkbox" name="blocks" value={b.key} /> <span className="text-[10px] uppercase text-muted2 w-16">{b.category}</span> {b.name}{b.vertical_key ? ` · ${b.vertical_key}` : ""}
              </label>
            ))}
          </div>
          <button className="btn-gold text-sm">Compilar & publicar nova versão</button>
          <p className="text-[11px] text-muted2">Publicar cria a próxima versão, marca como publicada e espelha na structure — tenants já provisionados não mudam.</p>
        </form>
      </section>
