...
export default async function Relacionamento({ searchParams }: { searchParams: Promise<{ canal?: string; filtro?: string; q?: string }> }) {
  ...
  const canalOn = isEmail ? await googleConfigured() : await zapiConfigured();
  const m = await currentMembership();

  const sb = await createClient();
  let query = sb.from("rel_conversas").select("id, assunto, contato_nome, contato_email, contato_phone, status, assigned_to, unread, last_message_at, client_id")
  ...
  return (
    <ContentArea>
      ...
      {canal === "whatsapp" && (
        <p className="ds-small mb-3">As conversas chegam pelo <b>webhook da Z-API</b> (não há botão de sincronizar). Responder e templates entram no <b>E4</b>.</p>
      )}
      {(
        <>
          {/* filtros + busca */}
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            ...
            <form className="flex gap-2" method="get">
              <input type="hidden" name="canal" value={canal} /><input type="hidden" name="filtro" value={filtro} />
