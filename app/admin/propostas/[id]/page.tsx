...
export default async function PropostaPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (p.status === "rascunho") {
    const [{ data: catalog }, { data: deals }] = await Promise.all([
    ...
    return (
      <div>
        ...
        <ProposalBuilder catalog={(catalog as CatalogItem[]) ?? []} deals={(deals as { id: string; title: string }[]) ?? []} proposal={p} />
      </div>
    );
  }
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="space-y-5">
          <div className="card p-6">
            ...
            <p className="text-xs text-muted2 mb-3">v{p.version}{p.sent_at ? ` · enviada ${new Date(p.sent_at).toLocaleDateString("pt-BR")}` : ""}</p>
