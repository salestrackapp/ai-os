    ...
    <ContentArea>
      <div className="max-w-3xl">
          ...
          {messages.map((m) => (
            <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${m.role === "user" ? "bg-gold text-white font-medium" : "bg-navy3 border border-line text-cream"}`}>
                {m.content}
                <span className="block text-[11px] mt-1 opacity-60">{m.role === "user" ? "Cliente" : m.autor === "humano" ? "Salestrack (humano)" : "Consultor (IA)"} · {new Date(m.created_at).toLocaleString("pt-BR")}</span>
              </div>
            </div>
          ))}
        ...
        <form action={postAdminReply.bind(null, id)} className="card p-4">
          ...
          <p className="text-[13px] text-muted2 mt-1">A mensagem é marcada como humana. No WhatsApp/Slack, é entregue ao cliente quando o canal está configurado.</p>
