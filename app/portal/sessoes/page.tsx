              ...
              {creditList.map((c) => {
                ...
                return (
                  <div key={c.id} className="bg-navy3 border border-line rounded-lg p-4">
                    ...
                    {c.valid_until && <p className="text-[13px] text-muted2 mt-1">válido até {new Date(c.valid_until).toLocaleDateString("pt-BR")}</p>}
                  </div>
                );
              })}
            </div>
              ...
              {catList.map((c) => (
                <div key={c.id} className="card p-5">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    ...
                    <span className="text-[11px] uppercase tracking-[.14em] text-muted2">{MARCA_LABELS[c.marca] ?? c.marca}</span>
                  </div>
                  ...
                  {c.calendly_url
                    ...
                    : <p className="text-[13px] text-muted2 mt-3">Agendamento via seu ponto focal.</p>}
                </div>
              ))}
            </div>
          </section>
