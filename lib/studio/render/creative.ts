import { isV2Accent, type CreativePayload, type CreativeSize, type CreativeTemplate, type CreativeSlide } from "@/lib/deliverables/types";
...
function canvasCss(size: CreativeSize, accent: string, bg?: string): string {
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .canvas{position:relative;width:${w}px;height:${h}px;overflow:hidden;color:#F4F4FA;font-family:'Montserrat',sans-serif;
    ${bg ? `background:#0B0B16;` : `background:radial-gradient(900px 600px at 78% -8%, ${accent}33, transparent), radial-gradient(700px 500px at -10% 108%, #EBF21214, transparent), #0B0B16;`}
  ...
  .cp{font-size:${isTall ? 40 : 34}px;color:#AFAFC6;margin-top:28px;line-height:1.4;max-width:${isTall ? "100%" : "78%"}}
  .q{font-size:${isTall ? 72 : 60}px;font-weight:800;font-style:italic;line-height:1.15;border-left:8px solid #EBF212;padding-left:36px}
  .au{font-size:30px;color:#AFAFC6;margin-top:32px;padding-left:44px}
  .big{font-size:${isTall ? 300 : 240}px;font-weight:900;line-height:.9;color:#EBF212;letter-spacing:-.05em}
  .bl{font-size:${isTall ? 46 : 40}px;color:#F4F4FA;margin-top:24px;font-weight:700}
  .bc{font-size:26px;color:#7A7A95;margin-top:14px}
  .big2{font-size:${isTall ? 96 : 80}px;font-weight:900;color:#EBF212;margin-top:28px}
  .bl2{font-size:32px;color:#AFAFC6;font-weight:600;letter-spacing:0}
  ...
  .spark{position:absolute;right:80px;top:76px;font-size:56px;color:#EBF212;z-index:2}`;
}
...
export function buildCreativeHtml(cr: CreativePayload, opts: CreativeRenderOpts = {}): string {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#0B0B16;color:#F4F4FA;font-family:'Montserrat',sans-serif;padding:28px}
    .meta{font-family:'JetBrains Mono',monospace;font-size:12px;color:#7A7A95;letter-spacing:.16em;text-transform:uppercase;margin-bottom:18px}
