...
function wrap(title: string, bodyHtml: string, cta?: { label: string; url: string }) {
  return `<!doctype html><html><body style="margin:0;background:#f4f1ea;font-family:Arial,Helvetica,sans-serif;padding:24px 12px">
  <div style="max-width:560px;margin:0 auto;background:#fffdf8;border:1px solid #e6e0d5;border-radius:12px;overflow:hidden">
    <div style="background:${NAVY};padding:20px 28px">
      <p style="margin:0;letter-spacing:.28em;text-transform:uppercase;color:${GOLD};font-size:10px">AI Operation System</p>
    </div>
    <div style="padding:28px">
      <h1 style="font-family:Georgia,serif;font-size:22px;color:${NAVY};margin:0 0 12px">${title}</h1>
      <div style="color:#333;font-size:14px;line-height:1.6">${bodyHtml}</div>
      ${cta ? `<p style="margin:24px 0 4px"><a href="${cta.url}" style="background:${GOLD};color:${NAVY};text-decoration:none;padding:12px 22px;border-radius:8px;font-weight:bold;font-size:14px;display:inline-block">${cta.label}</a></p>` : ""}
    </div>
    <div style="padding:16px 28px;border-top:1px solid #e6e0d5">
      <p style="margin:0;letter-spacing:.24em;text-transform:uppercase;color:${GOLD};font-size:9px;text-align:center">André Kachan · Salestrack AI</p>
    </div>
  </div></body></html>`;
}
